...
func TestTokenStore_CreateOrphanResponse(t *testing.T) {
	...
	resp, err := c.HandleRequest(namespace.RootContext(t.Context()), &logical.Request{
...
func TestTokenStore_TokenEntryUpgrade(t *testing.T) {
	...
	if err := ts.expiration.RegisterAuth(ctx, registryEntry, auth, "", true); err != nil {
		t.Fatal(err)
	}
	...
	if err := ts.create(ctx, ent, true); err != nil {
		t.Fatalf("err: %s", err)
	}
	auth = &logical.Auth{
	...
	if err := ts.expiration.RegisterAuth(ctx, ent, auth, "", true); err != nil {
		t.Fatal(err)
	}
	...
	if err := ts.expiration.RegisterAuth(nsCtx, ent, auth, "", true); err != nil {
		t.Fatal(err)
	}
	...
	if out.NamespaceID != ns.ID {
		t.Fatalf("bad: namespaceID: expected: %s, actual: %s", ns.ID, out.NamespaceID)
	}
	if !strings.HasSuffix(out.ID, ns.ID) {
		t.Fatalf("bad: ID doesn't end with namespace ID: %s, actual ID: %s", ns.ID, out.ID)
	}
...
func testMakeTokenViaRequest(t testing.TB, ts *TokenStore, req *logical.Request) *logical.Response {
	t.Helper()
	return testMakeTokenViaRequestContext(t, namespace.RootContext(t.Context()), ts, req)
}
...
func testMakeTokenViaRequestContext(t testing.TB, ctx context.Context, ts *TokenStore, req *logical.Request) *logical.Response {
	...
	if resp.Auth.TokenType != logical.TokenTypeBatch {
		if err := ts.expiration.RegisterAuth(ctx, te, resp.Auth, "", true); err != nil {
			t.Fatal(err)
		}
	}
...
func testMakeTokenDirectly(t testing.TB, ctx context.Context, ts *TokenStore, te *logical.TokenEntry) {
	...
	err := ts.expiration.RegisterAuth(ctx, te, auth, "", true)
	switch err {
	case nil:
...
func TestTokenStore_HandleRequest_LookupAccessor(t *testing.T) {
	...
	out, err := ts.Lookup(namespace.RootContext(t.Context()), "tokenid")
	if err != nil {
		t.Fatalf("err: %s", err)
	}
	if out == nil {
...
func TestTokenStore_HandleRequest_Renew_Revoke_Accessor(t *testing.T) {
	...
	rootToken, err := ts.rootToken(namespace.RootContext(t.Context()))
	if err != nil {
		t.Fatal(err)
	}
	root := rootToken.ID
	...
	te, err := ts.Lookup(namespace.RootContext(t.Context()), "tokenid")
	if err != nil {
		t.Fatal(err)
	}
	if te == nil {
	...
	err = exp.RegisterAuth(namespace.RootContext(t.Context()), te, auth, "", true)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	...
	_, err = ts.HandleRequest(namespace.RootContext(t.Context()), req)
	if err != nil {
		t.Fatalf("err: %s", err)
	}
...
func TestTokenStore_RootToken(t *testing.T) {
	...
	te, err := ts.rootToken(namespace.RootContext(t.Context()))
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	if te.ID == "" {
	...
	out, err := ts.Lookup(namespace.RootContext(t.Context()), te.ID)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
...
func TestTokenStore_CreateLookup_ExpirationInRestoreMode(t *testing.T) {
	...
	if err := ts.create(namespace.RootContext(t.Context()), ent, true); err != nil {
		t.Fatalf("err: %v", err)
	}
	if ent.ID == "" {
		t.Fatal("missing ID")
	...
	saltedID, err := ts.SaltID(namespace.RootContext(t.Context()), ent.ID)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	...
	if err := ts.expiration.persistEntry(namespace.RootContext(t.Context()), le); err != nil {
		t.Fatalf("err: %v", err)
	}

	out, err := ts.Lookup(namespace.RootContext(t.Context()), ent.ID)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	deepEqualTokenEntries(t, out, ent)
...
func TestTokenStore_Revoke(t *testing.T) {
	...
	if err := ts.create(ctx, entRoot, true); err != nil {
		t.Fatalf("err: %v", err)
	}

	entNS1 := &logical.TokenEntry{Path: "test", Policies: []string{"dev", "ops"}, NamespaceID: ns1.ID}
	if err := ts.create(namespace.ContextWithNamespace(ctx, ns1), entNS1, true); err != nil {
		t.Fatalf("err: %v", err)
	}

	entNS2 := &logical.TokenEntry{Path: "test", Policies: []string{"dev", "ops"}, NamespaceID: ns2.ID}
	if err := ts.create(namespace.ContextWithNamespace(ctx, ns2), entNS2, true); err != nil {
		t.Fatalf("err: %v", err)
	}
...
func TestTokenStore_Revoke_Leases(t *testing.T) {
	...
	leaseID, err := ts.expiration.Register(namespace.RootContext(t.Context()), req, resp, "")
	if err != nil {
		t.Fatalf("err: %v", err)
	}

	// Revoke the token
	err = ts.revokeOrphan(namespace.RootContext(t.Context()), ent.ID)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	...
	out, err := ts.expiration.loadEntry(namespace.RootContext(t.Context()), leaseID)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	if out != nil {
...
func TestTokenStore_HandleRequest_CreateToken_DisplayName(t *testing.T) {
	...
	out, err := ts.Lookup(namespace.RootContext(t.Context()), resp.Auth.ClientToken)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
...
func TestTokenStore_HandleRequest_CreateToken_RootID(t *testing.T) {
	...
	out, _ := ts.Lookup(namespace.RootContext(t.Context()), resp.Auth.ClientToken)
	if out.ID == "foobar" {
		t.Fatalf("bad: %#v", out)
	}
}
...
func TestTokenStore_HandleRequest_Revoke(t *testing.T) {
	...
	te, err := ts.Lookup(namespace.RootContext(t.Context()), "child")
	if err != nil {
		t.Fatal(err)
	}
	if te == nil {
	...
	te, err = ts.Lookup(namespace.RootContext(t.Context()), "sub-child")
	if err != nil {
		t.Fatal(err)
	}
	if te == nil {
	...
	out, err := ts.Lookup(namespace.RootContext(t.Context()), "child")
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	if out != nil {
	...
	out, err = ts.Lookup(namespace.RootContext(t.Context()), "sub-child")
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	if out != nil {
...
func TestTokenStore_HandleRequest_Renew(t *testing.T) {
	...
	// Create new token
	root, err := ts.rootToken(namespace.RootContext(t.Context()))
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	...
	err = exp.RegisterAuth(namespace.RootContext(t.Context()), root, auth, "", true)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
...
func TestTokenStore_HandleRequest_CreateToken_ExistingEntityAlias(t *testing.T) {
	...
	// Find mount accessor
	resp, err = core.systemBackend.HandleRequest(namespace.RootContext(t.Context()), &logical.Request{
		Path:      "auth",
		Operation: logical.ReadOperation,
	})
	if err != nil || (resp != nil && resp.IsError()) {
...
func TestTokenStore_RoleDisallowedPoliciesWithRoot(t *testing.T) {
	...
	resp, err = ts.HandleRequest(namespace.RootContext(t.Context()), roleReq)
	if err != nil || (resp != nil && resp.IsError()) {
		t.Fatalf("err:%v resp:%v", err, resp)
	}
...
func TestTokenStore_RoleTokenFields(t *testing.T) {
	...
	// c, _, root := TestCoreUnsealed(t)
	ts := c.tokenStore
	rootContext := namespace.RootContext(t.Context())
...
func TestTokenStore_NoDefaultPolicy(t *testing.T) {
	...
	resp, err = ts.HandleRequest(namespace.RootContext(t.Context()), tokenReq)
	if err != nil || (resp != nil && resp.IsError()) {
		t.Fatalf("err: %v, resp: %v", err, resp)
	}
	if !reflect.DeepEqual(resp.Auth.Policies, []string{"policy1"}) {
...
func TestTokenStore_AllowedDisallowedPolicies(t *testing.T) {
	...
	_, err = ts.HandleRequest(namespace.RootContext(t.Context()), tokenReq)
	if err == nil {
		t.Fatal("expected an error")
	}
...
func TestTokenStore_RevokeUseCountToken(t *testing.T) {
	...
	exp := mockExpiration(t)
	ts := exp.tokenStore
	root, _ := exp.tokenStore.rootToken(namespace.RootContext(t.Context()))
	...
	saltTut, err := ts.SaltID(namespace.RootContext(t.Context()), tut)
	if err != nil {
		t.Fatal(err)
	}
	te, err := ts.lookupInternal(namespace.RootContext(t.Context()), saltTut, true, false)
	if err != nil {
		t.Fatal(err)
	}
	if te == nil {
	...
	te, err = ts.UseToken(namespace.RootContext(t.Context()), te)
	if err != nil {
		t.Fatal(err)
	}
	if te == nil {
	...
	te, err = ts.lookupInternal(namespace.RootContext(t.Context()), saltTut, true, true)
	if err != nil {
		t.Fatal(err)
	}
	if te == nil {
	...
	err = ts.revokeInternal(namespace.RootContext(t.Context()), saltTut, false)
	if err == nil {
...
func TestTokenStore_Batch_NoCubbyhole(t *testing.T) {
	...
	te, err := ts.Lookup(namespace.RootContext(t.Context()), resp.Auth.ClientToken)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTokenStore_TokenID(t *testing.T) {
	t.Run("no custom ID provided", func(t *testing.T) {
		...
		resp, err := ts.HandleRequest(namespace.RootContext(t.Context()), &logical.Request{
