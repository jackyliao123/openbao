...
description: |-
  The PKCS#11 seal configures OpenBao to use an HSM with PKCS#11 as the
  auto-unseal mechanism.
---

# `pkcs11` seal

The PKCS#11 seal configures OpenBao to use an HSM with PKCS#11 as the auto-unseal
mechanism. OpenBao's PKCS#11 support is activated by one of the following:
...
expected that it is [seal-compatible](/community/policies/migration); manual
...
This example shows configuring a PKCS#11 seal through the OpenBao configuration
